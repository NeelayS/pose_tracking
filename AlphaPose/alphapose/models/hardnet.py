...
class CombConvLayer(nn.Sequential):
    def __init__(
        ...
        self.add_module(
            "layer2", DWConvLayer(out_channels, out_channels, norm_layer, stride=stride)
        )
...
class DWConvLayer(nn.Sequential):
    def __init__(self, in_channels, out_channels, norm_layer, stride=1, bias=False):
        ...
        if DEBUG:
            print(
                kernel, "x", kernel, "x", out_channels, "x", out_channels, "DepthWise"
            )
...
class HarDBlock_v2(nn.Module):
    ...
    def __init__(
        self, in_channels, growth_rate, grmul, n_layers, norm_layer, dwconv=False
    ):
        super().__init__()
        self.links = []
        conv_layers_ = []
...
class HarDNetBase(nn.Module):
    def __init__(self, arch, norm_layer, depth_wise=False):
        ...
        # Second Layer
        self.base.append(
            ConvLayer(first_ch[0], first_ch[1], norm_layer, kernel=second_kernel)
        )
        ...
        for i in range(blks):
            blk = HarDBlock(
                ch, gr[i], grmul, n_layers[i], norm_layer, dwconv=depth_wise
            )
            ch = blk.get_out_ch()
...
class HarDNetPose(nn.Module):
    def __init__(self, norm_layer=nn.BatchNorm2d, **cfg):
        ...
        self.conv_out = nn.Sequential(
            ...
            nn.Conv2d(
                ...
                stride=1,
                padding=cfg["FINAL_CONV_KERNEL"] // 2,
                bias=True,
            ),
        )
    ...
    def v2_transform(self):
        ...
        blk = self.last_blk
        self.last_blk = HarDBlock_v2(
            blk.in_channels, blk.growth_rate, blk.grmul, blk.n_layers, blk.norm_layer
        )
