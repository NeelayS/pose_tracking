...
class STrack(BaseTrack):
    ...
    def __init__(
        self, tlwh, score, temp_feat, pose, crop_box, file_name, ps, buffer_size=30
    ):
...
class Tracker(object):
    ...
    def update(
        ...
        if len(bboxs) > 0:
            detections = [
                STrack(STrack.tlbr_to_tlwh(tlbrs[:]), 0.9, f, p, c, file_name, ps, 30)
                for (tlbrs, f, p, c, ps) in zip(
                    bboxs, feats, pose, cropped_boxes, pscores
                )
            ]
        else:
            detections = []
