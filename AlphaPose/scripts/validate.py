...
parser.add_argument(
    "--checkpoint", help="checkpoint file name", required=True, type=str
)
...
def validate(m, heatmap_to_coord, batch_size=20):
    ...
    for inps, crop_bboxes, bboxes, img_ids, scores, imghts, imgwds in tqdm(
        ...
        if opt.flip_test:
            ...
            output_flip = flip_heatmap(
                m(inps_flip), det_dataset.joint_pairs, shift=True
            )
    ...
    for inps, labels, label_masks, img_ids, bboxes in tqdm(
        ...
        if opt.flip_test:
            if isinstance(inps, list):
                ...
                inps_flip = flip(inps).cuda()
            output_flip = flip_heatmap(
                m(inps_flip), gt_val_dataset.joint_pairs, shift=True
            )
            pred_flip = output_flip[:, eval_joints, :, :]
        else:
            output_flip = None
